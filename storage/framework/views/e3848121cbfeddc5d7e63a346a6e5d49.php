...
<?php $__env->startSection('title', 'Penjagaan vs Bulan Lalu'); ?>
...
<div class="space-y-6">
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <!-- Filter Form -->
        <form method="GET" action="<?php echo e(route('penerimaan.penjagaan.vsbulanlalu')); ?>" class="flex flex-wrap items-center gap-3">
            <select name="bulan" onchange="this.form.submit()" class="bg-slate-50 border border-slate-300 text-slate-700 text-xs rounded-xl focus:ring-blue-500 focus:border-blue-500 p-2.5 font-medium">
                <?php for($m=1; $m<=12; $m++): ?>
                    <option value="<?php echo e($m); ?>" <?php echo e($bulan == $m ? 'selected' : ''); ?>>
                        Bulan <?php echo e(DateTime::createFromFormat('!m', $m)->format('F')); ?>
            ...
            <?php if(request('fungsi') || request('jenis')): ?>
                <a href="<?php echo e(route('penerimaan.penjagaan.vsbulanlalu', ['bulan' => $bulan])); ?>" class="text-xs text-rose-500 hover:underline font-medium">Reset Filter</a>
            <?php endif; ?>
        </form>
    </div>
...
<script>
    ...
    new Chart(ctx, {
        type: 'line',
        data: {
            labels: <?php echo json_encode(array_map(fn($d) => "$d", $days)); ?>,
            datasets: [
                {
                    label: 'Bulan Ini (2026)',
                    data: <?php echo json_encode($dataBulanIni); ?>,
                ...
                {
                    label: 'Bulan Lalu',
                    data: <?php echo json_encode($dataBulanLalu); ?>,
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\mpnweb\resources\views/penerimaan/penjagaan/vs_bulan_lalu.blade.php ENDPATH**/ ?>