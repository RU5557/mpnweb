...
<header class="bg-slate-900 text-white px-8 py-4 flex items-center justify-between shadow-md">
        ...
        <div class="flex items-center gap-3">
            <a href="<?php echo e(url('/penerimaan')); ?>" class="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm px-4 py-2 rounded-lg transition border border-slate-700 font-medium">
    ...
    <main class="max-w-6xl mx-auto p-8 space-y-8">
        ...
        <section class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <form action="<?php echo e(route('admin.rolling-text.update')); ?>" method="POST" class="p-6 space-y-6">
                <?php echo csrf_field(); ?>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Tanggal Info</label>
                        <input type="date" name="tanggal" value="<?php echo e(old('tanggal', $rollingText->tanggal ?? date('Y-m-d'))); ?>" required
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="nko" value="<?php echo e(old('nko', $rollingText->nko ?? 0)); ?>" placeholder="Contoh: 95.40" required
                            class="w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Ranking Nasional</label>
                        <input type="number" name="ranking_nasional" value="<?php echo e(old('ranking_nasional', $rollingText->ranking_nasional ?? 0)); ?>" placeholder="Contoh: 12" required
                            class="w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Ranking Kanwil</label>
                        <input type="number" name="ranking_kanwil" value="<?php echo e(old('ranking_kanwil', $rollingText->ranking_kanwil ?? 0)); ?>" placeholder="Contoh: 2" required
                            class="w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="pesan_tambahan" value="<?php echo e(old('pesan_tambahan', $rollingText->pesan_tambahan ?? '')); ?>" placeholder="Contoh: Tetap Semangat Menjaga Penerimaan Negara!"
                        class="w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                </div>
        ...
        <section class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="font-bold text-slate-800 text-base">Update Target Tahunan (Tahun <?php echo e($tahunSekarang); ?>)</h2>
                </div>
                <span class="bg-emerald-100 text-emerald-800 text-xs px-2.5 py-1 rounded-full font-semibold">Statik Tahunan</span>
            </div>

            <form action="<?php echo e(route('admin.target.update')); ?>" method="POST" class="p-6 space-y-6">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="tahun" value="<?php echo e($tahunSekarang); ?>">

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Target Kantor (Global)</label>
                        <input type="text" name="target_kantor" value="<?php echo e(old('target_kantor', isset($target->target_kantor) ? number_format($target->target_kantor, 0, ',', '.') : 0)); ?>" required
                    ...
                    <div>
                        ...
                        <input type="text" name="target_ppm" value="<?php echo e(old('target_ppm', isset($target->target_ppm) ? number_format($target->target_ppm, 0, ',', '.') : 0)); ?>" required
                            class="rupiah-input w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Target PKM (Total)</label>
                        <input type="text" name="target_pkm" value="<?php echo e(old('target_pkm', isset($target->target_pkm) ? number_format($target->target_pkm, 0, ',', '.') : 0)); ?>" required
                            class="rupiah-input w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Target PBP</label>
                        <input type="text" name="target_pbp" value="<?php echo e(old('target_pbp', isset($target->target_pbp) ? number_format($target->target_pbp, 0, ',', '.') : 0)); ?>" required
                            class="rupiah-input w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Target PKM Pengawasan</label>
                        <input type="text" name="target_pkm_pengawasan" value="<?php echo e(old('target_pkm_pengawasan', isset($target->target_pkm_pengawasan) ? number_format($target->target_pkm_pengawasan, 0, ',', '.') : 0)); ?>" required
                            class="rupiah-input w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Target PKM Pemeriksaan</label>
                        <input type="text" name="target_pkm_pemeriksaan" value="<?php echo e(old('target_pkm_pemeriksaan', isset($target->target_pkm_pemeriksaan) ? number_format($target->target_pkm_pemeriksaan, 0, ',', '.') : 0)); ?>" required
                            class="rupiah-input w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase mb-2">Target PKM Penagihan</label>
                        <input type="text" name="target_pkm_penagihan" value="<?php echo e(old('target_pkm_penagihan', isset($target->target_pkm_penagihan) ? number_format($target->target_pkm_penagihan, 0, ',', '.') : 0)); ?>" required
                            class="rupiah-input w-full bg-slate-50 border border-slate-300 text-slate-800 text-sm rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium">
                    </div>
                </div>
...
</html><?php /**PATH C:\xampp\htdocs\mpnweb\resources\views\admin\index.blade.php ENDPATH**/ ?>