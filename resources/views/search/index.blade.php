...
@section('content')
    <div class="space-y-6" x-data="{
        ...
        <!-- ==================== FORM PENCARIAN ==================== -->
        <div class="bg-white p-5 rounded-2xl p-5 pt-0 shadow-sm border border-slate-200/80">
            <form id="searchForm" action="{{ route('wp.search') }}" method="GET" @submit="loading = true" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-3 items-start">
                    ...
                    <div class="md:col-span-3">
                        ...
                        <select name="target_table" x-model="targetTable" @change="$nextTick(() => $el.form.submit())"
                            ...
                            <option value="masterfile" {{ $targetTable === 'masterfile' ? 'selected' : '' }}>Masterfile WP
                            </option>
                            <option value="detil_transaksi" {{ $targetTable === 'detil_transaksi' ? 'selected' : '' }}>Detil
                                DRM</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-7">
                        <label class="block text-xs font-semibold text-slate-600 mb-1.5">
                            Kata Kunci
                            <span
                                x-text="targetTable === 'masterfile' ? '(NPWP15 / NPWP16 / Nama WP)' : '(NPWP15 / Nama WP)'"
                                class="text-slate-400 font-normal"></span>
                        </label>
                        <div class="relative">
                            <input type="text" name="q" value="{{ $keyword ?? '' }}"
                                :placeholder="targetTable === 'masterfile' ? 'Masukkan NPWP15, NPWP16, atau Nama WP...' :
                                    'Masukkan NPWP15 atau Nama WP...'"
                    ...
                    <div class="md:col-span-2 flex flex-col justify-end h-full">
                        <div class="hidden md:block h-[22px]"></div>
                        <!-- Spacer pengganti label agar tombol turun sejajar input -->
                ...
                <div x-show="targetTable === 'detil_transaksi'" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-3 items-center">
                        ...
                        @endphp
                        <div x-data="{ open: false, selectAll: {{ $isAllThn ? 'true' : 'false' }}, selected: {{ json_encode($currentThn) }}, options: {{ json_encode($listTahunArr) }} }" x-init="$watch('selected', value => selectAll = (value.length === options.length && options.length > 0))" class="relative">
                            ...
                            <button type="button" @click="open = !open"
                                ...
                                <span
                                    x-text="selected.length === options.length && options.length > 0 ? 'Semua Tahun Terpilih' : (selected.length ? selected.length + ' Tahun Dipilih' : 'Tidak Ada Dipilih')"></span>
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak
                                class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-xl p-3 max-h-60 overflow-y-auto">
                                <label
                                ...
                                @foreach ($listTahunSetor ?? [] as $thn)
                                    <label
                                        ...
                                        <input type="checkbox" name="thn_setor[]" value="{{ $thn }}"
                                            x-model="selected">
                                        <span>{{ $thn }}</span>
                                    </label>
                                @endforeach
                        ...
                        @endphp
                        <div x-data="{ open: false, selectAll: {{ $isAllBulan ? 'true' : 'false' }}, selected: {{ json_encode($currentBln) }}, allKeys: {{ json_encode($allBulanKeys) }} }" x-init="$watch('selected', value => selectAll = value.length === allKeys.length)" class="relative">
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Bulan Setor</label>
                            <button type="button" @click="open = !open"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl p-2 text-left text-xs text-slate-800 flex justify-between items-center">
                                <span
                                    x-text="selected.length === allKeys.length ? 'Semua Bulan Terpilih' : (selected.length ? selected.length + ' Bulan Dipilih' : 'Tidak Ada Dipilih')"></span>
                                <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7"></path>
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak
                                ...
                                <label
                                    class="flex items-center gap-2 font-bold text-xs pb-2 border-b border-slate-100 cursor-pointer">
                                    <input type="checkbox" x-model="selectAll"
                                        @change="selected = selectAll ? [...allKeys] : []"> Pilih Semua
                                </label>
                                @foreach ($bulanList as $key => $val)
                                    <label
                                        class="flex items-center gap-2 text-xs py-1 cursor-pointer hover:bg-slate-50 px-1 rounded">
                                        <input type="checkbox" name="bln_setor[]" value="{{ $key }}"
                                            x-model="selected">
                                        <span>{{ $val }}</span>
                                    </label>
                                @endforeach
                        ...
                        @endphp
                        <div x-data="{ open: false, selectAll: {{ $isAllFungsi ? 'true' : 'false' }}, selected: {{ json_encode($currentFungsi) }}, options: {{ json_encode($listFungsiArr) }} }" x-init="$watch('selected', value => selectAll = (value.length === options.length && options.length > 0))" class="relative">
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Fungsi</label>
                            <button type="button" @click="open = !open"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl p-2 text-left text-xs text-slate-800 flex justify-between items-center">
                                <span
                                    x-text="selected.length === options.length && options.length > 0 ? 'Semua Fungsi Terpilih' : (selected.length ? selected.length + ' Fungsi Dipilih' : 'Tidak Ada Dipilih')"></span>
                                <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7"></path>
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak
                                ...
                                @foreach ($listFungsi ?? [] as $f)
                                    <label
                                        class="flex items-center gap-2 text-xs py-1 cursor-pointer hover:bg-slate-50 px-1 rounded">
                                        <input type="checkbox" name="fungsi[]" value="{{ $f }}"
                                            x-model="selected">
                                        <span>{{ $f }}</span>
                                    </label>
                                @endforeach
                        ...
                        @endphp
                        <div x-data="{ open: false, selectAll: {{ $isAllAr ? 'true' : 'false' }}, selected: {{ json_encode($currentAr) }}, options: {{ json_encode($listArArr) }} }" x-init="$watch('selected', value => selectAll = (value.length === options.length && options.length > 0))" class="relative">
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Nama AR</label>
                            <button type="button" @click="open = !open"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl p-2 text-left text-xs text-slate-800 flex justify-between items-center">
                                <span
                                    x-text="selected.length === options.length && options.length > 0 ? 'Semua AR Terpilih' : (selected.length ? selected.length + ' AR Dipilih' : 'Tidak Ada Dipilih')"></span>
                                <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7"></path>
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak
                                class="absolute z-50 mt-1 w-72 bg-white border border-slate-200 rounded-xl shadow-xl p-3 max-h-60 overflow-y-auto">
                                <label
                                    class="flex items-center gap-2 font-bold text-xs pb-2 border-b border-slate-100 cursor-pointer">
                                    <input type="checkbox" x-model="selectAll"
                                        @change="selected = selectAll ? [...options] : []"> Pilih Semua
                        ...
                        @endphp
                        <div x-data="{ open: false, selectAll: {{ $isAllJs ? 'true' : 'false' }}, selected: {{ json_encode($currentJs) }}, options: {{ json_encode($listJsArr) }} }" x-init="$watch('selected', value => selectAll = (value.length === options.length && options.length > 0))" class="relative">
                            <label class="block text-xs font-semibold text-gray-600 mb-1">Nama JS</label>
                            <button type="button" @click="open = !open"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl p-2 text-left text-xs text-slate-800 flex justify-between items-center">
                                <span
                                    x-text="selected.length === options.length && options.length > 0 ? 'Semua JS Terpilih' : (selected.length ? selected.length + ' JS Dipilih' : 'Tidak Ada Dipilih')"></span>
                                <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M19 9l-7 7-7-7"></path>
            ...
            <div x-show="!loading" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-visible">
                ...
                <div
                    ...
                    <span class="text-xs font-medium text-slate-600">
                        Hasil Pencarian di <strong
                            class="text-slate-900 font-bold">{{ $targetTable === 'masterfile' ? 'Masterfile WP' : 'Detil DRM' }}</strong>
                        (Total: <span
                    ...
                    @if ($targetTable === 'masterfile')
                        ...
                        <table class="w-full text-left text-sm text-slate-700">
                            <thead
                                ...
                                <tr>
                                    <th class="p-3.5">
                                        <a href="{{ sortUrl('npwp15', $sortBy, $sortOrder) }}"
                                            class="flex items-center gap-1.5 hover:text-blue-600">
                                            NPWP15 / NPWP16
                                            @if (($sortBy ?? '') === 'npwp15')
                                                <i
                                                    class="fa-solid fa-arrow-{{ $sortOrder === 'asc' ? 'up' : 'down' }} text-blue-600"></i>
                                            @endif
                                        </a>
                                    </th>
                                    <th class="p-3.5">
                                        <a href="{{ sortUrl('nama', $sortBy, $sortOrder) }}"
                                            class="flex items-center gap-1.5 hover:text-blue-600">
                                            Nama Wajib Pajak
                                            @if (($sortBy ?? '') === 'nama')
                                                <i
                            ...
                            <tbody class="divide-y divide-slate-100">
                                    ...
                                    <tr class="hover:bg-blue-50/40 transition odd:bg-white even:bg-slate-50/50">
                                        ...
                                        <td class="p-3.5 text-xs leading-relaxed max-w-xs">
                                            ...
                                            @if ($item->kecamatan || $item->kota)
                                                <div class="text-slate-400 text-[11px]">{{ $item->kecamatan }},
                                                    {{ $item->kota }}</div>
                                            @endif
                                        </td>
                                        <td class="p-3.5 whitespace-nowrap">
                                            <div class="font-semibold text-slate-700 text-xs mb-0.5">
                                                {{ $item->jenis_wp ?? '-' }}</div>
                                            <span
                                                class="bg-slate-200/80 text-slate-700 px-2 py-0.5 rounded-md text-[10px] font-bold">{{ $item->status_wp ?? '-' }}</span>
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="p-8 text-center text-slate-400 italic">Data Masterfile
                                            tidak ditemukan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    @else
                        ...
                        <table class="w-full text-left text-sm text-slate-700">
                            <thead
                                ...
                                <tr>
                                    <th class="p-3.5 whitespace-nowrap">
                                        <a href="{{ sortUrl('tgl_setor', $sortBy, $sortOrder) }}"
                                            class="flex items-center gap-1.5 hover:text-blue-600">
                                            Tgl Setor
                                    ...
                                    <th class="p-3.5">
                                        <a href="{{ sortUrl('fungsi', $sortBy, $sortOrder) }}"
                                            class="flex items-center gap-1.5 hover:text-blue-600">
                                            Fungsi
                                            @if (($sortBy ?? '') === 'fungsi')
                                                <i
                                    ...
                                    <th class="p-3.5">
                                        <a href="{{ sortUrl('kd_map', $sortBy, $sortOrder) }}"
                                            class="flex items-center gap-1.5 hover:text-blue-600">
                                            MAP / Bayar
                                            @if (($sortBy ?? '') === 'kd_map')
                                                <i
                                    ...
                                    <th class="p-3.5 text-right whitespace-nowrap">
                                        <a href="{{ sortUrl('jml_setor', $sortBy, $sortOrder) }}"
                                            class="flex items-center justify-end gap-1.5 hover:text-blue-600">
                                            Jumlah Setor (Rp)
                                            @if (($sortBy ?? '') === 'jml_setor')
                                                <i
                                                    class="fa-solid fa-arrow-{{ $sortOrder === 'asc' ? 'up' : 'down' }} text-blue-600"></i>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="p-8 text-center text-slate-400 italic">Data Transaksi
                                            tidak ditemukan.</td>
                                    </tr>
                                @endforelse
                            </tbody>
