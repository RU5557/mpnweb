...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-xs text-slate-500 mt-0.5">Top 10 Wajib Pajak kontributor terbesar pencairan tunggakan (Pencairan STP/SKP)</p>
    </div>
...
<!-- TABEL TOP 10 WP PKM PENAGIHAN -->
<div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
    <div class="p-5 border-b border-slate-100 flex items-center justify-between">
        <div class="flex items-center gap-2">
            <div class="w-2.5 h-2.5 rounded-full bg-purple-600"></div>
            <h2 class="text-sm font-bold text-slate-800">10 Wajib Pajak Terbesar PKM Penagihan</h2>
        </div>
        <span class="bg-purple-100 text-purple-700 text-[11px] px-2.5 py-0.5 rounded-full font-bold">Top 10</span>
    </div>
        ...
        <table class="w-full text-left text-xs">
            <thead class="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-200">
                <tr>
                    ...
                    <th class="py-3.5 px-4">Tindakan Penagihan</th>
                    <th class="py-3.5 px-4 text-right">Pencairan Tunggakan (Rp)</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 text-slate-700 font-medium">
                @forelse($topWpPenagihan as $index => $row)
                    <tr class="hover:bg-slate-50/80 transition">
                        ...
                        <td class="py-3 px-4 text-slate-600">
                            <span class="bg-slate-100 text-slate-700 px-2 py-0.5 rounded text-[11px] border border-slate-200">
                                {{ $row->tindakan ?? 'Penagihan Aktif' }}
                            </span>
                        </td>
                        <td class="py-3 px-4 text-right font-mono font-bold text-purple-600">Rp {{ number_format($row->total_realisasi ?? 0, 0, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="py-8 text-center text-slate-400 italic">Tidak ada data PKM Penagihan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
