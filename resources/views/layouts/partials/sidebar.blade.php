    ...
    <div>
        ...
        <nav class="p-3 space-y-1.5">
            ...
            <a href="{{ route('penerimaan.dashboard') }}" 
               class="flex items-center gap-3.5 px-3.5 py-2.5 rounded-xl text-[13px] font-semibold transition-colors {{ $isDashboard ? 'bg-blue-600 text-white shadow-md shadow-blue-600/30' : 'hover:bg-slate-800 hover:text-white text-slate-300' }}">
            ...
            <div x-data="{ open: {{ $isPenjagaan ? 'true' : 'false' }} }" class="space-y-1">
                <button @click="if(!sidebarOpen && !isPinned) { sidebarOpen = true; open = true; } else { open = !open; }" 
                        :class="open ? 'bg-slate-800 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white'"
                    ...
                    <i x-show="sidebarOpen || isPinned" x-cloak class="fa-solid text-xs transition-transform duration-200" :class="open ? 'fa-chevron-down' : 'fa-chevron-right'"></i>
                </button>
                ...
                <div x-show="open && (sidebarOpen || isPinned)" 
                    ...
                    <a href="{{ route('penerimaan.penjagaan.bulanan') }}" 
                       class="block px-3 py-2 rounded-lg text-xs font-medium transition-colors {{ request()->routeIs('penerimaan.penjagaan.bulanan') ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-white hover:bg-slate-800/60' }}">
                        Bulanan
                    </a>
