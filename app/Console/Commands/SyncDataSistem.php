...
{
    /**
     * Signature command dengan opsi target filter
     */
    protected $signature = 'sync:data-sistem {--only=all : Pilihan target: all, ref, master, tx}';

    protected $description = 'ETL data dari mpninfo (legacy) ke database fresh operasional';
    ...
    public function handle()
    {
        ...
        $this->info("====================================================");
        $this->info("  MEMULAI ETL DATA LEGACY (Mode: {$target})");
        $this->info("====================================================");
        ...
        try {
            ...
            // 2. Sync Masterfile WP
            if (in_array($target, ['all', 'master'])) {
                $this->syncMasterfileWp();
            }

            // 3. Sync Transaksi Detail & Auto Rebuild Summary
            if (in_array($target, ['all', 'tx'])) {
                $this->syncDetilTransaksiWp();
                
                $this->newLine();
                $this->comment('-> Memicu otomatis kalkulasi summary mart...');
            ...
            $this->newLine();
            $this->error("ETL ERROR: " . $e->getMessage());
